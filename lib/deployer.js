(function (module, JSON) {
    ...
    function run(logger, config, cb) {
        console.log("running");
        return cb();
    }

    function execute(logger, config, provision, cb) {
        ...
        return loadFile(logger, merge(config, parsed), function(err, config) {
            ...
            return provision(logger, config.app, function(err) {
                if (err) {
                    return cb(err);
                }
    ...
    module.exports = function(cb) {
        var cfg = arguments[1] || config,
            prvson = arguments[2] || provision; // for unit testing
        ...
        return cfg(function(err, config) {
            ...
            return execute(logger, config, prvson, function(err) {
                if (logger.awsLogging) {
