(function (module, JSON) {
    ...
    function parseUserData(logger, data, cb) {
        if (data === '{') {
    ...
    function loadFile(logger, config, SSS, cb) {
        if (!config.file) {
            return cb(new Error("no configuration file specified"));
        }
        ...
        if (!matched) {
            return cb(new Error("unknown protocol used for " + config.file));
        }
    ...
    function execute(logger, config, provision, run, SSS, cb) {
        return parseUserData(logger, config["user-data"], function(err, parsed) {
            if (err) {
                return cb(err);
            }

            return loadFile(logger, merge(config, parsed), SSS, function (err, config) {
                if (err) {
                    return cb(err);
                }
