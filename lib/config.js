    ...
    function loadEC2Config(cb) {
        ...
        batch.push(function(cb) {
            return value("meta-data/iam/security-credentials", function(err, val) {
                ...
                return value("meta-data/iam/security-credentials/" + value, function(err, value) {
                    if (value) {
                        var val = JSON.parse(value);

                        result["aws-access-key-id"] = val.AccessKeyId;
                        result["aws-secret-access-key"] = val.SecretAccessKey;
                    }
